...
//=======
            ...
            if showComments && !viewModel.showReportPopup {
                CommentsPopupView(
                    postId: selectedPostID, onCancel: {
                        showComments = false
                        tabRouter.isTabBarHidden = false
                    },
                    onReportTapped: {
            ...
            ForEach(viewModel.events, id: \.postId) { event in
                ...
                DiscoverEventCard(
                    ...
                    onReportPostTap: {
                        ...
                        viewModel.saveUnsaveApi(postId: event.postId, index: index, postType: "Event")
        ...
        VStack(spacing: 20) {
            ForEach(viewModel.activities) { activity in
                ...
                ActivitiesCardView(
                    ...
                    onCommentTap: {
                        selectedPostID = activity.postId ?? ""
                        showComments = true
                        tabRouter.isTabBarHidden = true
                    },
                    ...
                    onReportPostTap: {
                        selectedPostID = activity.id
                        viewModel.showPostReportPopUp = true
                        tabRouter.isTabBarHidden = true
                    },
                    onLikeTap: { _ in
                        viewModel.likeUnlikeActivityApi(
                            postId: activity.postId ?? "",
                            index: index,
                            postType: "Post"
                        )
                    ...
                    onFollowTap: {
                        viewModel.FollowUnfollowApi(index: activity.userId ?? "", Index1: index)
                    },
                    
                    onSaveTap: {
                        viewModel.saveUnsaveApi(postId: activity.postId ?? "", index: index, postType: "Activity")
                        }
                )
                .onAppear {
                    if index == viewModel.activities.count - 1 {
